    ...
    <div class="p-3 lg:ml-64 lg:pt-3">
        <div id="contentDiv" class="p-2 w-full">
            <div class="bg-white overflow-hidden shadow-md rounded-lg p-3">
                <form action="{{ route('menu.category.update') }}" method="POST" enctype="multipart/form-data" class="grid">
                    @csrf
                    <input type="hidden" name="slug" value="{{ $category->slug }}">
                    <label class="relative inline-flex items-center cursor-pointer mb-3">
                        <input {{ ($category->is_hidden == 1) ? 'checked' : '' }} type="checkbox" name="hidden" id="hidden" value="1" class="sr-only peer">
                    ...
                    <div class="flex justify-between">
                        <a href="{{ route('menu.category.index') }}" class="text-white w-1/2 max-w-xs bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none tracking-wider font-semibold text-center">BACK</a>
